...
export default function AsidePanel({isStart, projects, onSelectSideProject, selectedID, languages})  {

    return (
            <aside className="w-1/4 bg-aside-gradient bg-fixed h-full rounded-tr-2xl text-white font-sans shadow-2xl">
               <h1 className="pt-14 pb-10 pl-8 uppercase text-stone-800 text-3xl font-bold font-sans">{languages.asideTitle}</h1>
                <div>
                    <button onClick={isStart} className="rounded-lg mb-4 opacity-70 bg-stone-600 ml-8 px-6 py-3 shadow-2xl text-xl hover:opacity-100 transition-opacity">+ {languages.asideButton}</button>
                </div>
                <ul className="my-6 ">
